        ...
        <div class="w-full">
            ...
            <div class="bg-white text-lg rounded border border-gray-200 p-5 pt-4 m-5 mt-4">
                <div class="flex items-center gap-5">
                    @if ($category->name)
                        <img src="{{ $category->image_url }}" alt="{{ $category->name }}" class="w-[120px] h-[120px] border border-gray-200 rounded-full">
                    @endif
                ...
                <div class="mt-4">
                    <form action="{{ route('dashboard.categories.destroy', $category->id) }}" method="POST" onsubmit="return confirm('Сигурни ли сте, че искате да изтриете тази категория?');">
                        @csrf
                        @method('DELETE')
