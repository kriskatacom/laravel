            ...
            <div class="bg-white text-lg rounded border border-gray-200 p-5 pt-4 m-5 mt-4">
                <div class="flex items-center gap-5">
                    ...
                    <div class="flex flex-col">
                        ...
                        <div>{{ $user->created_at->translatedFormat('d F Y, H:i') }}</div>
                    </div>
                </div>
                <div class="mt-4">{{ $user->description }}</div>
                <div class="flex gap-5 mt-4">
                    <a href="{{ route('dashboard.users.index', $user->id) }}" class="primary-button">
                        Назад
                    </a>
                    <a href="{{ route('dashboard.users.edit', $user->id) }}" class="warning-button">
                        Редакция
                    </a>
                    @if (Auth::user()->email !== $user->email)
                        <form action="{{ route('dashboard.users.destroy', $user->id) }}" method="POST"
                            ...
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="danger-button">Изтрий потребител</button>
                        </form>
                    @endif
                </div>
            </div>
